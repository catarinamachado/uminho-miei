...
Description : Módulo Haskell que contém um exemplo de possível reação de um comando num estado de jogo

Um módulo contendo funções recursivas e não recursivas com o intuito de devolver um mapa atualizado consoante comandos.
-}
...
-- | 'move' : função que devolve um novo estado de jogo consoante os __comandos__
...
linhaplanta m j = "*" ++ " " ++ (coordenadasJ j m) ++ " " ++ [j] ++ " " ++ raioBomb m j ++ " " ++ "10"

--passa raio (Int) para raio String
-- | 'raioBomb' : função que altera o o tipo do raio de [String] para String

raioBomb :: [String] -> Char -> String
raioBomb m j = show (raioB m j) 
